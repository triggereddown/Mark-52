...
const ProjectCard = ({ project, index }) => (
  <motion.div
    ...
    className="group bg-black/40 backdrop-blur-md border border-white/5 rounded-xl overflow-hidden hover:border-white/20 hover:shadow-2xl hover:shadow-cyan-500/10 transition-all duration-300 flex flex-col h-full"
  >
    {/* Image Container */}
    ...
    <div className="p-5 flex-1 flex flex-col">
      ...
      <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-white/5">
        {project.tech?.slice(0, 4).map((t) => (
          <span key={t} className="text-xs font-medium text-cyan-200/80 bg-cyan-900/20 px-2 py-1 rounded border border-cyan-500/10">
            {t}
          </span>
        ))}
      ...
      <div className="mb-8 flex flex-col md:flex-row justify-between items-end gap-4 border-b border-white/10 pb-6">
        <div>
           ...
           <p className="text-gray-400 max-w-lg">
             A collection of full-stack applications, solving real-world problems with modern technologies.
            </p>
        </div>
        <div className="text-sm text-gray-500 font-mono">
           {myProjects.length} Projects Found
        </div>
      </div>
